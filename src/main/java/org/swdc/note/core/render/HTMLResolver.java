...
public class HTMLResolver extends ContentRender implements FileExporter {
    ...
    @Override
    public void writeFile(Article article, Path path) {
        try {
    ...
    public void writeType(ArticleType type, Path file) {
        ...
        metadata.addAuthor(new Author("Fantastic Note"));
        metadata.addDate(new Date(new java.util.Date()));
        metadata.setLanguage("zh-CN");
        metadata.addDescription("Fantastic Note Generated Document.");
        metadata.addPublisher("Fantastic Note Generated");
        metadata.addTitle(type.getName());

        Set<Article> articles = type.getArticles();
        for (Article article: articles) {
            byte[] data = renderAsBytes(article);
            Resource resource = new Resource(data, "/content/" + article.getId());
            resource.setMediaType(new MediaType("html", "html"));
            book.addSection(article.getTitle(), resource);
        }
        ...
        try {
            ...
            writer.write(book,Files.newOutputStream(file));
        } catch (Exception e) {
            logger.error("fail to write epub file",e);
        }
    }

    @Override
    public Article readFile(Path filePath) {
    ...
    @Override
    public ArticleType readTypeFile(Path path) {
        return null;
    }
